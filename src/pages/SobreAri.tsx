...
export const SobreAriPage: React.FC = () => {
  return (
    <section className="relative z-10 py-12 md:py-16 px-4 max-w-5xl mx-auto flex flex-col gap-14">
      {/* breadcrumbs */}
      <nav className="text-sm text-green-800/70">
        <Link to="/" className="hover:text-green-900">
          Inicio
        </Link>
        <span className="mx-2">/</span>
        <span className="text-pink-600">Sobre Ari</span>
      </nav>

      {/* hero */}
      <header className="text-center space-y-4">
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-green-900">
          Sobre <span className="text-pink-600">Ari</span>
        </h1>
        <p className="text-green-800 text-base sm:text-lg md:text-xl max-w-2xl mx-auto leading-relaxed">
      ...
      <section className="flex flex-col md:flex-row items-center gap-8">
        ...
        <article className="flex-1 text-green-800 space-y-4 text-sm sm:text-base">
          <p>
          ...
          <p>
            Mi misión con <strong>Entre Pétalos</strong> es traer esa belleza
      ...
      {/* misión / valores */}
      <section className="bg-white/30 backdrop-blur-sm rounded-2xl shadow-md p-8 space-y-6">
        <h2 className="text-2xl sm:text-3xl font-semibold text-green-900">
          Nuestra misión
        </h2>
        <ul className="list-disc list-inside text-green-800 leading-relaxed space-y-1">
      ...
      <div className="text-center">
        <Link
          to="/contacto"
          className="inline-block px-6 sm:px-8 py-3 bg-pink-600 hover:bg-pink-500 text-white rounded-full transition"
        >
          ¡Hablemos!
        </Link>
      </div>
